...
from rules.rule import WoundRerollModifier, WoundRollModifier
...
class HitWound:
    def __init__(self, attacker: Unit, target: Unit):
        ...
        self._strategy: WoundStrategy = attacker._wound_strategy
        ...
        self._roll_modifiers = [
            m for m in offensive_modifiers if isinstance(m, WoundRollModifier)
        ] + [m for m in defensive_modifiers if isinstance(m, WoundRollModifier)]

        # Combine reroll modifiers from both attacker and target
        self._reroll_modifiers = [
            m for m in offensive_modifiers if isinstance(m, WoundRerollModifier)
        ] + [m for m in defensive_modifiers if isinstance(m, WoundRerollModifier)]

    def wound_target(self) -> bool:
        ...
        if self._strategy.calculate_wound(self._attacker, self._target, roll):
            return True

        # If the initial roll failed, check if we should reroll
        if self._should_reroll(roll):
            roll = self._roll_and_apply_modifiers()
            return self._strategy.calculate_wound(self._attacker, self._target, roll)
